...
applied to the gateway city routing fractions (Equation~\ref{eq:routing});
Newark and New York are subsequently merged into a single New York
entry, yielding five canonical gateway cities:
\begin{equation}
...
\begin{itemize}
  ...
  by city pair and travel date. These would replace the I-92 regional
  routing fractions and COR monthly aggregates with direct
  country-to-city routing specific to the WC travel window, including
  routing to the eleven venue cities not currently covered by I-92 data.
  ...
  Qatar 2022 — obtained through host country immigration authorities or
  public health agencies — would allow empirical validation of the
  schedule-based routing fractions used in Model 3 and calibration of
  the WC-specific growth factors.
...
\begin{itemize}
  ...
  adults (18--50 years). Age-specific incidence data, available from
  WHO and national surveillance systems for several diseases, would
  allow weighting country-level burden by the age distribution of the
  ...
  estimates (Immunization Data portal) would allow adjustment of
  effective incidence to account for immunity in the traveling
  population, replacing the implicit assumption that travelers have
  the same vaccination status as the general population.
...
\begin{itemize}
  ...
  cities, available through CDC ArboNET and state health department
  surveillance programmes, would allow extension of the framework
  to estimate the probability of a local dengue outbreak following
  an importation event.

  \item \textbf{Healthcare system capacity.}
  Hospital capacity, infectious disease bed availability, and public
  health rapid-response capacity data by host city (American Hospital
  Association annual survey, state health departments) would
  contextualise the importation risk estimates for local health
  system planning and resource pre-positioning.
