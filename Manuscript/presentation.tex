    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item \textbf{I-92 programme} (BTS/USDOC): monthly arrivals
              by world region to 5 US gateway cities — \emph{baseline
              model only}
        \item \textbf{BTS T-100 International Segment}: nonstop
              ...
              \textbf{11 US venue cities}; Models 2 \& 3
...
\begin{frame}{Three-model hierarchy}
  ...
  \begin{tikzpicture}[
    ...
    \node[below=0.4cm of m2, font=\footnotesize, text width=3.8cm, align=center]
      {Same diseases\\country-level\\T-100 routing};
...
\begin{frame}{Model 2 — WC-adjusted: travel volume construction}
  ...
  \[
    ...
    \qquad\qquad
    N_{c,h}^{2026} = N_{c,\,\text{Jun 2026}} \cdot f_{c,h}^{\text{T100}}
  \]

  \smallskip
  {\footnotesize $f_{c,h}^{\text{T100}}$: T-100 country-level routing fraction
  (mean June 2023--2025). Covers \textbf{all 11 US venue cities}, replacing
  I-92 regional fractions (5 cities only).}
\end{frame}
...
\begin{frame}{Limitations}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item T-100 covers all 11 US venue cities for the
              background stream (upgrade from I-92)
        \item 5 non-US venues (Canada, Mexico) still receive
              only the WC-fan stream — background risk
              underestimated there
        \item T-100 captures nonstop segments only; connecting
              passengers appear at entry city, not final destination
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Model scope}
    ...
    \begin{column}{0.33\textwidth}
      ...
      \begin{itemize}\small
        \item \textbf{\textit{Ae.~aegypti} surveillance} (CDC
              ArboNET) — extend model from importation to
              local dengue outbreak probability
        \item \textbf{Hospital capacity} (AHA survey) —
              contextualise risk for health system
              resource pre-positioning
        \item \textbf{Mobility data} (Meta Movement, Google) —
              validate the schedule-based fan routing
...
\begin{frame}{WC fans vs.\ background — malaria, measles, pertussis}
  ...
  {\footnotesize All four diseases. \textcolor{wcadj}{$\blacksquare$ WC fans} \quad
   ...
   WC-fan increment $\approx 3.4\%$ of projected arrivals for qualified nations.}
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}{Sensitivity analysis — relative $\Lambda$}
  \begin{center}
    \includegraphics[width=\textwidth, height=0.68\textheight, keepaspectratio]%
      {../Figures/sensitivity_analysis_relative.png}
  \end{center}
  \vspace{-0.3em}
  {\footnotesize Relative sensitivity = $\Lambda_{\text{perturbed}} /
   \Lambda_{\text{central}}$. Uniform across all cities and diseases:
   city/disease \textbf{rankings are robust} to parameter uncertainty
   even though absolute values are not.}
\end{frame}
