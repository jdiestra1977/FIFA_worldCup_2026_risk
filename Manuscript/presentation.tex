...
\begin{frame}{Disease-specific parameters}
  ...
  \begin{tabular}{lcccl}
    ...
    Influenza
      & 0.033--0.20 & \textbf{0.10} & 0.50
      & Moderate illness; many travel during early illness \\
    Malaria
      & 0.10--0.35 & \textbf{0.20} & 0.30
      & Severe illness; most febrile after return \\
    Measles
      & 0.40--0.80 & \textbf{0.60} & 0.05
  ...
  WHO GISRS / Iuliano et al.\ 2018 (influenza);
  WHO WMR 2024 (malaria); Simons et al.\ 2012 (measles);
  McLaughlin et al.\ 2016 (pertussis).
  MC: 5,000 Uniform draws on each $(\rho_d,\, p_d)$ pair.}
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}{Monte Carlo uncertainty propagation}
  Both $\rho_d$ and $p_d$ span \textbf{one or more orders of magnitude}
  in the literature.

  \bigskip
  \textbf{Approach}: 5,000 independent draws from
  $\rho_d \sim \text{Uniform}(\rho_{\min},\rho_{\max})$ and
  $p_d \sim \text{Uniform}(p_{\min},p_{\max})$.
  Because $\Lambda \propto \rho_d \cdot p_d$, each draw rescales the
  central estimate by a scalar factor — \textit{no recomputation needed}.

  \bigskip
  \begin{block}{Why this matters}
    \begin{itemize}
      \item Dengue/influenza 95\% CI extends \textbf{upward}: risk may
            be underestimated
      \item Pertussis 95\% CI extends \textbf{downward}: central estimate
            is a conservative upper bound ($\rho_c = \rho_{\max}$)
      \item Uncertainty is \textbf{integrated into every result}, not
            relegated to a sensitivity appendix
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Importation risk overview — all five diseases}
  ...
  5,000 draws). Dengue is near-certain at every venue;
  measles risk is lowest. Cities ordered by decreasing aggregate
...
\begin{frame}{Key findings}
  \begin{enumerate}
    \item \textbf{Dengue importation is near-certain} ($P(\geq 1)>0.99$
          at every US venue city) — robust across the full MC parameter
          space.
    \item \textbf{Influenza ranks second}, amplified by coincidence
          with the Southern Hemisphere winter peak during the June
          tournament window.
...
\begin{frame}{Next steps}
  \begin{enumerate}
    \item \textbf{Incorporate knockout rounds} — as the WC bracket
          is resolved, update the schedule-driven model with actual
          match assignments beyond the group stage
    ...
    \item \textbf{Expand disease scope} — chikungunya (active 2025--2026
          outbreak in Brazil \& Argentina) and mumps are the
          highest-priority additions; both have accessible public data
