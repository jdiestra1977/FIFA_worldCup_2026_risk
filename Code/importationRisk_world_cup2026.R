# ============================================================
# FIFA World Cup 2026 - Supplementary Analyses
# Author: Jose Herrera-Diestra
# ------------------------------------------------------------
# Depends on objects produced by importationRisk_main.R.
# Run source("Code/importationRisk_main.R") before this file.
# ============================================================
...
dengue_data_world_selected <- dengue_data_world %>%
  ...
  left_join(correspondence_country_region %>% select(country = Country, new_region), by = "country")

malaria_cases <- read_csv("Data/Malaria_National_Unit_data.csv") %>%
...
# ============================================================
# 1. COR arrivals: total to US by country, July-August
# ============================================================
# The FIFA world cup 2026 will be hosted by three countries: USA, Mexico and Canada.
# Most games will happen in USA. Dates: June 11 - July 16.
...
total_arrivals_July_August_by_country <- arrivals_COR %>%
  ...
  pivot_longer(cols = -c(Country, World_region), names_to = "year_month", values_to = "value") %>%
...
total_arrivals_July_August_by_country <- total_arrivals_July_August_by_country %>%
  mutate(Country = if_else(
    Country == "Zaire ( formerly Congo, Democratic Republic of)",
    "Zaire (formerly DRC)", Country)) %>%
  mutate(new_world_region = case_when(
...
df_plot <- total_arrivals_July_August_by_country %>%
  select(-World_region) %>%
  mutate(year = as.character(year), total = as.numeric(total)) %>%
  filter(year == "2025", total > 0) %>%
...
# ============================================================
# 2. Assign County + FIPS to BTS-ranked airports
# ============================================================
# Outputs:
...
#   - data_with_prob_of_entry    (airport entry probabilities)
# ============================================================

review_threshold <- 0.12

# Manually verified wrong matches
bad_airport_raw <- c(
...
air_us <- airports_info %>%
  ...
  mutate(state = str_replace(iso_region, "^US-", ""),
         airport_key = norm_airport(name)) %>%
  select(name, iata_code, latitude_deg, longitude_deg, state, airport_key)

ranking2 <- ranking2 %>%
  mutate(state_block = case_when(
    state == "DC"                                                 ~ "DC|VA|MD",
...
match_one <- function(airport_key, state_block) {
  ...
  if (nrow(cand) == 0 || is.na(airport_key) || airport_key == "") {
    return(tibble(matched_airport_name = NA_character_, iata_code = NA_character_,
                  latitude_deg = NA_real_, longitude_deg = NA_real_, match_dist = NA_real_))
  }
  d <- stringdist::stringdist(airport_key, cand$airport_key, method = "jw")
  j <- which.min(d)
  tibble(matched_airport_name = cand$name[j], iata_code = cand$iata_code[j],
         latitude_deg = cand$latitude_deg[j], longitude_deg = cand$longitude_deg[j],
         match_dist = d[j])
}
...
matched_clean <- ranking2 %>%
  ...
  unnest_wider(tmp) %>%
  ungroup() %>%
  select(Airport, Enplaned = 3, airport_raw, city, state, state_block,
         matched_airport_name, iata_code, latitude_deg, longitude_deg, match_dist)
...
bad %>% drop_na(match_dist) %>% arrange(match_dist) %>% print(n = 50)
...
matched_clean_filtered %>%
  summarise(n_total = n(), max_dist = max(match_dist, na.rm = TRUE),
...
counties_sf <- tigris::counties(cb = TRUE, year = 2023) %>%
  select(county_name = NAME, county_fips = GEOID, state_fips = STATEFP) %>%
  st_transform(4326)

airports_with_county_fips <- st_join(airports_sf, counties_sf, join = st_within) %>%
  st_drop_geometry() %>%
  select(Airport, Enplaned, airport_raw, city, state,
...
airports_with_county_fips %>%
  summarise(n_total = n(), n_with_county = sum(!is.na(county_fips)),
...
# ============================================================
# 3. Map: airport entry probabilities + World Cup venues
# ============================================================
...
pad_x <- (bb$xmax - bb$xmin) * 0.02

venues_position <- read_csv("Data/world_cup_2026_stadiums_coordinates.csv") %>%
  filter(country == "USA") %>%
  rename("latitude_deg" = "latitude", "longitude_deg" = "longitude")

venues_labels <- venues_position %>%
  transmute(venue = stadium, lon = longitude_deg, lat = latitude_deg) %>%
  mutate(
...
move_label <- function(df, venue_name, new_lon = NULL, new_lat = NULL) {
  df %>%
    mutate(
      label_lon = ifelse(venue == venue_name & !is.null(new_lon), new_lon, label_lon),
      label_lat = ifelse(venue == venue_name & !is.null(new_lat), new_lat, label_lat)
    )
}
...
ggplot() +
  ...
  geom_point(data = venues_position,
    aes(x = longitude_deg, y = latitude_deg), color = "red", size = 2) +
  ...
  theme(legend.position = "top", legend.title = element_text(size = 15),
        legend.text = element_text(size = 12), plot.margin = margin(0, 5, 0, 30))

ggsave(last_plot(), file = "Figures/map_probs_and_venues.png")

# ============================================================
# 4. Country-level disease importation (COR mobility data)
# ============================================================
...
visits_and_population_countries <- total_arrivals_July_August_by_country %>%
  ...
  left_join(population_of_world) %>%
  drop_na() %>%
  rename("total_visits_to_US" = "total") %>%
  select(-year)

# Dengue: total cases June-August from most recent complete year per country
dengue_JJA_by_country_year <- dengue_data_world_selected %>%
  drop_na() %>%
  mutate(year = year(date), month = month(date)) %>%
  filter(month %in% c(6, 7, 8)) %>%
  group_by(country, year) %>%
  summarise(cases_JJA = sum(cases, na.rm = TRUE), n_months = n(), .groups = "drop") %>%
  arrange(country, year) %>%
  rename("year_of_cases" = "year", "Country" = "country")

dengue_data_world_clean <- dengue_JJA_by_country_year %>%
  filter(n_months == 3) %>%
  group_by(Country) %>%
  slice_max(year_of_cases) %>%
  mutate(Country = recode(Country,
    "Venezuela (Bolivarian Republic of)" = "Venezuela",
    "Bolivia (Plurinational State of)"   = "Bolivia",
    "Iran (Islamic Republic of)"         = "Iran",
...
exp_imp_dengue <- expected_imp_dengue_country_region %>%
  ...
  geom_col() +
  scale_y_log10(name = "Expected dengue importations",
    breaks = 10^seq(-5, 3), labels = trans_format("log10", math_format(10^.x))) +
...
prob_imp_dengue <- expected_imp_dengue_country_region %>%
  filter(expected_imp_by_country > 0) %>%
  head(n = 30) %>%
  ggplot(aes(x = Country, y = prob_at_least_one, fill = new_world_region)) +
  geom_col() +
  scale_fill_manual(values = cb_palette_named) + coord_flip() + theme_bw() +
...
dengue_map <- ggplot() +
  ...
  geom_point(data = expected_imp_by_airport %>%
    ...
    aes(x = longitude_deg, y = latitude_deg,
        size = expected_importations, fill = expected_importations),
    shape = 21, color = "black", stroke = 1, alpha = 0.6) +
  scale_size_continuous(name = "Expected dengue importations", range = c(1.5, 8)) +
  ...
  guides(size = "none",
         fill = guide_colourbar(title = "Expected dengue importations")) +
...
measles_country_import <- visits_and_population_countries %>%
  left_join(measles_data %>% select(Country = 1, cases_per1M = 4) %>%
              drop_na() %>% mutate(cases_per1M = as.numeric(cases_per1M)),
            by = "Country") %>%
...
exp_imp_measles <- measles_country_import %>%
...
prob_imp_measles <- measles_country_import %>%
  filter(exp_import_country > 0) %>%
  mutate(Country = fct_reorder(Country, exp_import_country)) %>%
  arrange(desc(exp_import_country)) %>%
  head(n = 30) %>%
...
ggsave(last_plot(), file = "Figures/figure_measles.png", width = 20, height = 15)
...
measles_country_airport <- measles_country_import %>%
  ...
  mutate(expected_measles_importations = exp_import_country * prob_entry)

measles_by_airport <- measles_country_airport %>%
  group_by(Airport, latitude_deg, longitude_deg, Enplaned, prob_entry) %>%
  summarise(expected_measles_importations = sum(expected_measles_importations), .groups = "drop")
...
measles_map <- ggplot() +
  ...
  geom_point(data = measles_by_airport %>%
    ...
    aes(x = longitude_deg, y = latitude_deg,
        size = expected_measles_importations, fill = expected_measles_importations),
    shape = 21, color = "black", stroke = 1, alpha = 0.6) +
...
malaria_country_import <- visits_and_population_countries %>%
  ...
  mutate(cases_per1K = as.numeric(cases_per1K),
         exp_import_country = (cases_per1K / 1000) * total_visits_to_US) %>%
  mutate(prob_at_least_one = 1 - exp(-exp_import_country)) %>%
  drop_na(exp_import_country)

exp_imp_malaria <- malaria_country_import %>%
  filter(exp_import_country > 0) %>%
  mutate(Country = fct_reorder(Country, exp_import_country)) %>%
  arrange(desc(exp_import_country)) %>%
  head(n = 30) %>%
...
prob_imp_malaria <- malaria_country_import %>%
  filter(exp_import_country > 0) %>%
  mutate(Country = fct_reorder(Country, exp_import_country)) %>%
  arrange(desc(exp_import_country)) %>%
  head(n = 30) %>%
...
malaria_country_airport <- malaria_country_import %>%
  ...
  tidyr::crossing(data_with_prob_of_entry %>%
    select(Airport, latitude_deg, longitude_deg, Enplaned, prob_entry)) %>%
  mutate(expected_malaria_importations = exp_import_country * prob_entry)

malaria_by_airport <- malaria_country_airport %>%
  group_by(Airport, latitude_deg, longitude_deg, Enplaned, prob_entry) %>%
  summarise(expected_malaria_importations = sum(expected_malaria_importations), .groups = "drop")
...
pertussis_country_import <- visits_and_population_countries %>%
  left_join(pertussis_data %>% select(Country = 1, cases_per1M = 4) %>%
              drop_na() %>% mutate(cases_per1M = as.numeric(cases_per1M)),
            by = "Country") %>%
  mutate(cases_per1M = as.numeric(gsub(",", "", cases_per1M)),
         exp_import_country = (cases_per1M / 1e6) * total_visits_to_US) %>%
...
exp_imp_pertussis <- pertussis_country_import %>%
  filter(exp_import_country > 0) %>%
  mutate(Country = fct_reorder(Country, exp_import_country)) %>%
  arrange(desc(exp_import_country)) %>%
  head(n = 30) %>%
...
prob_imp_pertussis <- pertussis_country_import %>%
  filter(exp_import_country > 0) %>%
  mutate(Country = fct_reorder(Country, exp_import_country)) %>%
  arrange(desc(exp_import_country)) %>%
  head(n = 30) %>%
...
plot_grid(exp_imp_dengue, exp_imp_measles, exp_imp_malaria, exp_imp_pertussis,
          ncol = 4, labels = "AUTO", label_size = 30)
ggsave(last_plot(), file = "Figures/expected_importations_top30.png", width = 25, height = 7)

# ============================================================
# 8. US Cities from Census (ACS)
# ============================================================
# census_api_key("YOUR_KEY_HERE", install = TRUE)  # run once to set key
# readRenviron("~/.Renviron")

cities_us <- get_acs(
  geography = "place",
  variables = c(pop = "B01003_001"),
  year = 2023, survey = "acs5", geometry = TRUE
) %>%
...
contig_states     <- setdiff(sprintf("%02d", c(1:56, 72)), c("02", "15", "72"))
cities_us_contig  <- cities_us     %>% filter(substr(GEOID, 1, 2) %in% contig_states)
cities_us_pts_contig <- cities_us_pts %>% filter(substr(GEOID, 1, 2) %in% contig_states)

ggplot() +
  geom_sf(data = cities_us_contig, fill = "grey95", color = "white", linewidth = 0.05) +
  geom_point(data = cities_us_pts_contig,
    aes(x = lon, y = lat, size = population, color = population), alpha = 0.6) +
  scale_size_continuous(name = "Population", range = c(0.3, 6),
    breaks = c(1e5, 5e5, 1e6, 5e6), labels = scales::comma) +
  scale_color_viridis_c(name = "Population", option = "plasma",
...
# ============================================================
# 9. Old mobility approach: individual arrivals by port-of-entry city
# (Data_International_arrivals - legacy; Selected_cities_and_origins is preferred)
# ============================================================
...
files_df <- tibble(file_path = file_vec) %>%
  mutate(
    ...
    city_query  = city_slug %>% str_replace_all("_", " ") %>% str_squish() %>% str_to_lower(),
    city_pretty = str_to_title(city_query)
  ) %>%
  filter(!is.na(city_slug))

air_us_international <- airports_info %>%
  filter(iso_country %in% c("US", "PR", "VI", "GU", "MP", "AS"),
         type %in% c("large_airport", "medium_airport"),
         str_detect(name, regex("international", ignore_case = TRUE)))
...
find_airport_matches <- function(city_query, strict_international = TRUE) {
  ...
  out <- base_tbl %>%
    ...
    transmute(airport_name = name, municipality, iso_region, iso_country, type,
              iata_code, icao_code, latitude_deg, longitude_deg)
  if (nrow(out) == 0) {
    out <- tibble(airport_name = NA_character_, municipality = NA_character_,
                  iso_region = NA_character_, iso_country = NA_character_,
                  type = NA_character_, iata_code = NA_character_,
                  icao_code = NA_character_, latitude_deg = NA_real_, longitude_deg = NA_real_)
  }
  out
}
...
potential_airport_matches <- airport_candidates %>%
  drop_na(airport_name, iata_code) %>%
  filter(iso_country == "US")

# Match file cities to Census places
cities_lu <- cities_us_contig %>%
  mutate(city_name = NAME %>%
           str_to_lower() %>%
           str_remove(",.*$") %>%
           str_remove("\\s+(city|town|village)$") %>%
           str_squish()) %>%
...
teams_dates_venues %>%
  ...
  ggplot(aes(x = City, fill = Country)) +
  geom_bar() + theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = c(0.1, 0.8))

# Countries with highest ticket demand
# Source: https://dentonrc.com/sports/fifa-to-start-notifying-winners-of-world-cup-ticket-lottery/
# More than 500M requests. Top countries (excl. hosts): Germany, England, Brazil,
# Spain, Portugal, Argentina, Colombia.
countries_most_demand_tickets <- c("Argentina", "Germany", "England", "Colombia",
                                   "Brazil", "Spain", "Portugal", "Ecuador", "Scotland")

rbind(
  teams_dates_venues %>% select(-`Team 2`) %>% rename("Team" = "Team 1"),
  teams_dates_venues %>% select(-`Team 1`) %>% rename("Team" = "Team 2")
) %>%
  filter(Team %in% countries_most_demand_tickets) %>%
  rename("match_date" = "Match Date") %>%
  arrange(match_date)

# Estimated length of stay by region:
# Latin & South America: 16 nights | Europe: 14 nights | Asia Pacific: 13 nights
